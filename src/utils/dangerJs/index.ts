import { DangerObjectModel, LogFunction } from './DangerModel'
export const checkPRAssigned = async (danger:DangerObjectModel, fail:LogFunction) => {
  if (danger?.github?.pr?.assignees.length === 0) {
    fail('PR must be assigned to somebody!! 🙏')
  }
}

export const checkChangedFiles = async (danger:DangerObjectModel, warn:LogFunction, limit = 10) => {
...
export const checkTicketLinkInPrBoby = async (danger:DangerObjectModel, fail:LogFunction) => {
...
export const checkNewDependencies = async (danger: DangerObjectModel, warn:LogFunction) => {
...
export const checkUpdatedTests = (danger: DangerObjectModel, warn:LogFunction, testFilePattern = 'test') => {
